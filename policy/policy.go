...
func (p PasswordAuthSession) GetSession() (auth tpm2.Session, closer func() error, err error) {
	...
	return tpm2.HMACSession(p.rwr, tpm2.TPMAlgSHA256, 16, tpm2.Auth(p.password), tpm2.AESEncryption(128, tpm2.EncryptIn))
...
func NewDefaultSession(rwr transport.TPM, encryptionHandle tpm2.TPMHandle) (DefaultSession, error) {
	return DefaultSession{rwr, encryptionHandle}, nil
}

func (p DefaultSession) GetSession() (auth tpm2.Session, closer func() error, err error) {
	var ePubName *tpm2.TPMTPublic
	if p.encryptionHandle != 0 {
		encryptionPub, err := tpm2.ReadPublic{
			ObjectHandle: p.encryptionHandle,
